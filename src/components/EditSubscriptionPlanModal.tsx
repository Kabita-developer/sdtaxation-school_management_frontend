...
import { SubscriptionPlan, CreateSubscriptionPlanRequest } from '../services/mockService';

interface EditSubscriptionPlanModalProps {
  isOpen: boolean;
  onClose: () => void;
  onSubmit: (planId: string, planData: Partial<CreateSubscriptionPlanRequest>) => Promise<void>;
...
export default function EditSubscriptionPlanModal({
  ...
  const validateForm = (): boolean => {
    ...
    if (formData.planName !== undefined) {
    ...
    if (formData.maxEmployees !== null && formData.maxEmployees !== undefined && formData.maxEmployees < 1) {
      newErrors.maxEmployees = 'Max employees must be at least 1 or null for unlimited';
    }

    if (formData.maxAdmins !== undefined && formData.maxAdmins < 1) {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await onSubmit(plan._id, formData);
    } catch (error) {
          ...
          <div className="flex items-center justify-between">
            <div className="flex items-center space-x-3">
              ...
              <div>
                ...
                <p className="text-blue-100 text-sm">{plan.planName}</p>
              </div>
            </div>
            <button
              onClick={onClose}
